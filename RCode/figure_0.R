...
p_d <- ggplot(data=res_d[res_d$a_date>as.POSIXct(as.Date("2008-07-01")),], aes(x=a_date,y=lines_added)) + geom_line() + 
  scale_y_log10("Added lines") + scale_x_datetime("Time") + ggtitle("Android OMAP, Added new lines, DAILY")
...
p_w <- ggplot(data=res_w[res_w$a_date>as.POSIXct(as.Date("2008-07-01")),], aes(x=a_date,y=lines_added)) + geom_line() + 
  scale_y_log10("Added lines") + scale_x_datetime("Time") + ggtitle("Android OMAP, Added new lines, WEEKLY")
...
p_m <- ggplot(data=res_m[res_m$a_date>as.POSIXct(as.Date("2008-07-01")),], aes(x=a_date,y=lines_added)) + geom_line() + 
  scale_y_log10("Added lines") + scale_x_datetime("Time") + ggtitle("Android OMAP, Added new lines, MONTHLY")
...
print(arrangeGrob(p_d, p_w, p_m, ncol=1))
dev.off()
ggsave(arrangeGrob(p_d, p_w, p_m, ncol=1), file="figures/omap_added_lines_plot.eps", width=16, height=10)
