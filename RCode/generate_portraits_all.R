...
get_series =function(id, start, end){
  ...
  res=rep(0,length(dates)-1)
  for(i in c(1:(length(dates)-1))){
    ll <- dbGetQuery(con, 
        paste("select sum(c.added_lines) lines_added from OMAP.change c ",
        "where c.author_id=",id, " and ",                 
        "c.project_id=1 and DATE_FORMAT(c.author_date, \"%Y-%m-%d\")=",
        shQuote(dates[i]),sep=""))
    if(!(is.na(ll))){
      res[i]=unlist(ll)
    }
...
pre_res=cbind(pre_res,as.data.frame(t(get_series(7377,as.Date("2013-02-04"),as.Date("2013-02-11")))))
...
post_res=cbind(post_res,as.data.frame(t(get_series(7377,as.Date("2013-02-04"),as.Date("2013-02-11")))))
...
for(k in c(1:(length(releases$date)))){
  ...
  for(i in c(1:4)){
   ...
   new.dt_pre_end    <- monday_before - as.difftime(i-1, unit="weeks")
   new.dt_post_start <- monday_after + as.difftime(i-1, unit="weeks")
   new.dt_post_end   <- monday_after + as.difftime(i, unit="weeks")
   print(paste(new.dt_pre_start, new.dt_pre_end))
   print(paste(new.dt_post_start, new.dt_post_end))
   # PRE-RELEASE
   # users
   users=dbGetQuery(con, 
      paste("select distinct(c.author_id), cp.email from OMAP.change c ",
      "join OMAP.change_people cp on c.author_id=cp.id where ",                 
      "c.project_id=1 and DATE_FORMAT(c.author_date, \"%Y-%m-%d\") between ",
...
write.table(pre_res,file="pre_portraits.txt", quote=F, col.names=T, row.names=F, sep="\t")
write.table(post_res,file="post_portraits.txt", quote=F, col.names=T, row.names=F, sep="\t")
