...
pre_res=data.frame(release_id=7,tag="pre",id=3450,email="dimitrysh@google.com",start=as.Date("2013-02-03"),end=as.Date("2013-02-10"))
pre_res=cbind(pre_res,as.data.frame(t(get_series(3450,as.Date("2013-02-04"),as.Date("2013-02-10")))))
...
for(k in c(1:(length(releases$date)))){
  ...
  for(i in c(1:4)){
   ...
   users=dbGetQuery(con, 
      ...
      "join android.change_people cp on c.author_id=cp.id where ",                 
      "c.project_id=18 and c.added_lines>0 and ",
      "DATE_FORMAT(c.author_date, \"%Y-%m-%d\") between ",
...
write.table(pre_res,file="omap_pre_portraits.txt", quote=F, col.names=T, row.names=F, sep="\t")
write.table(post_res,file="omap_post_portraits.txt", quote=F, col.names=T, row.names=F, sep="\t")
...
write.table(pre_res,file="omap_pre_portraits_monthly.txt", quote=F, col.names=T, row.names=F, sep="\t")
write.table(post_res,file="omap_post_portraits_monthly.txt", quote=F, col.names=T, row.names=F, sep="\t")
